...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        console.warn("Supabase signin failed, falling back to mock login:", error.message);
        // Initialize settings with email if empty or different
        const localSettings = mockStore.getSettings();
        if (localSettings.email !== email) {
          const fallbackName = email.split("@")[0].split(/[._\-+]/).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ") || "User";
      ...
      console.warn("Supabase signin error, falling back to mock login:", err);
      const localSettings = mockStore.getSettings();
      if (localSettings.email !== email) {
        const fallbackName = email.split("@")[0].split(/[._\-+]/).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ") || "User";
        mockStore.saveSettings({
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative animate-fadeIn">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <p className="mt-2 text-center text-sm text-slate-400">
          Or{" "}
          <Link href="/signup" className="font-medium text-primary hover:text-primary/95 transition duration-200">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs">
                <a href="#" className="font-medium text-slate-400 hover:text-foreground transition">
                  Forgot password?
                </a>
              </div>
            </div>
